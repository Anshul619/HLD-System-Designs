...
- Leader (master) copy is maintained by [leader-follower (master-slave)](../../4_Consistency&Replication/Replication.md) and replicated to n other follower (slave) nodes.
...
| [Scalability (Read Queries)](../../ScalabilityDB.md)                  | Under extremely high read load, you could balance the `reads between the master and follower`.<br/>- Although it would be recommended to setup up additional `no persistence, memory only` follower for that purpose. |
