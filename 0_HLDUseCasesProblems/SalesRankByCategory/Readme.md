...
> Note - `id`, `category_id`, `product_id` columns should be [indexed](../../3_Databases/5_Database-Internals/Indexing.md), for faster performance.
...
# Scaling the design
- To address the 40,000 average read requests per second (higher at peak), traffic for popular content (and their sales rank) should be handled by the [Redis Cache](../../3_Databases/8_Caching-InMemory-Databases/Redis/Readme.md) instead of the database.
