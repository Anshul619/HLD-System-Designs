...
- We can use any off-the-shelf solution like [Redis](../../3_Databases/8_InMemory-Databases/Redis/Readme.md), which can store full URLs with their respective hashes.
...
## Cache Memory - Redis
- `KGS` can always keep some keys in memory ([Redis](../../3_Databases/8_InMemory-Databases/Redis/Readme.md)) to quickly provide them whenever a server needs them.
...
This approach can still lead to overloaded partitions, which can be solved using [Consistent Hashing](../../3_Databases/3_ScalabilityTechniques/PartitioningSharding/Readme.md).
