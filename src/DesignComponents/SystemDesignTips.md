...
- If it's a WRITE heavy microservice ( `HIGH throughput` ), the best decision would be to use either [Kafka](MessageBrokers/Kafka.md) ( as message queue ) or [DynamoDB](AWS/DatabaseServices/DynamoDB.md) ( as data store ).
...
## Use [AWS S3](AWS/StorageServices/AmazonS3.md) for files storage
- AWS S3 is cost-effective and quite fast.
