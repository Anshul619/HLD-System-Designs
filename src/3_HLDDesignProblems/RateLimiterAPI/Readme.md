...
- We can store the timestamp of each request in a [Redis Sorted Set](../../1_HLDDesignComponents/3_DatabaseComponents/In-Memory-DB/Redis/Readme.md) in our ‘value’ field of hash-table.
...
- We can store our counters in a [Redis Hash](../../1_HLDDesignComponents/3_DatabaseComponents/In-Memory-DB/Redis/Readme.md) since it offers incredibly efficient storage for fewer than 100 keys. 
...
- In distributed systems, Rate limit can be implemented using [Redis (as centralized data store, to solve inconsistency problem)](../../1_HLDDesignComponents/3_DatabaseComponents/In-Memory-DB/Redis/Readme.md)

![img.png](assets/HLD%20-%20RateLimiter.drawio.png)

Note:
- If we are using [Redis](../../1_HLDDesignComponents/3_DatabaseComponents/In-Memory-DB/Redis/Readme.md) to store our key-value, one solution to resolve the atomicity problem is to use [Redis lock](../../1_HLDDesignComponents/3_DatabaseComponents/In-Memory-DB/Redis/Readme.md) for the duration of the read-update operation.
...
## How would it work?
- When a request is made, a new temporary record is stored in [Redis](../../1_HLDDesignComponents/3_DatabaseComponents/In-Memory-DB/Redis/Readme.md). 
...
### Race Conditions
- To solve the race condition while updating the counter in [Redis](../../1_HLDDesignComponents/3_DatabaseComponents/In-Memory-DB/Redis/Readme.md), we would have to apply [transaction locks on the read-write operation](../../1_HLDDesignComponents/3_DatabaseComponents/In-Memory-DB/Redis/Readme.md#redis-transaction-lock).
- This would make the [counter update as atomic](../../1_HLDDesignComponents/3_DatabaseComponents/1_Glossaries/ACIDTransactions/Atomicity.md) in Redis.
- But this comes at a performance cost ( as latency would increase ).

### Inconsistency
- Using the consistent data store (like [Redis](../../1_HLDDesignComponents/3_DatabaseComponents/In-Memory-DB/Redis/Readme.md), [Cassandra](../../1_HLDDesignComponents/3_DatabaseComponents/NoSQL-Databases/WideColumnDB/ApacheCasandra.md)), we can solve inconsistency problem of current rate limiting in distributed systems.
