...
- [Nodes with controlplane]((https://kubernetes.io/docs/concepts/overview/components/)) role run the K8s master components (excluding [etcd](../6b_ClusterCoordinationService/etcd.md), as its separate role). 
...
| [etcd](../6b_ClusterCoordinationService/etcd.md)                                                    | etcd is used for [Configuration Store & Service Discovery](../1_MicroServicesSOA/2_ServiceRegistry&Discovery).<br/>- etcd is a [Consistent](../0_SystemGlossaries/Database/ReplicationAndDataConsistency.md) and [highly-available key value store](../0_SystemGlossaries/Reliability/HighAvailability.md) used as Kubernetes backing store for all cluster data. |
...
## K8s Networking
- K8s manages its own load balancer, service discovery (through [etcd](../6b_ClusterCoordinationService/etcd.md)) etc.
